...
with tab1:
        ...
        fig_s = px.scatter(df, x="mobility_score", y="avg_pain", 
                          color="pain_status" if 'pain_status' in df.columns else None, 
        ...
        try:
            ...
            pred_val = model.predict(pd.DataFrame([p_data[feats]]))[0]
            pred = round(float(pred_val), 1)
            
            c_m1, c_m2 = st.columns([1, 2])
            c_m1.metric("예상 통증 지수 (VAS)", f"{pred} / 10")
            with c_m2:
                if pred > 6.0: st.error("🚨 중증도 통증 위험! 즉각적인 가동성 개선이 필요합니다.")
                elif pred > 3.0: st.warning("⚠️ 경미한 통증 예상. 꾸준한 스트레칭을 권장합니다.")
                else: st.success("✅ 통증 지수 안정적. 현재 밸런스를 유지하세요.")
        except:
            pred = "N/A"
            st.info("ℹ️ AI 모델 정보를 불러올 수 없습니다.")
        ...
        with cv_l:
            ...
            fig_r = go.Figure()
            fig_r.add_trace(go.Scatterpolar(r=[100]*6, theta=[info['name'] for info in joints_map.values()], 
                                          fill='none', name='정상 기준', line=dict(color='rgba(150,150,150,0.5)', dash='dash')))
            fig_r.add_trace(go.Scatterpolar(r=percent_vals, theta=[info['name'] for info in joints_map.values()], 
                                          fill='toself', name='달성도(%)', fillcolor=fill_color, line=dict(color=theme_color, width=3), 
                                          customdata=actual_vals, hovertemplate='<b>%{theta}</b><br>달성도: %{r:.1f}%<br>실제: %{customdata}°<extra></extra>'))
            fig_r.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 115])), showlegend=False, margin=dict(t=30, b=30))
        ...
        with cv_r:
            ...
            for j in joints:
                ...
                st.markdown(f"""<div style="background-color: {card_color}; padding: 10px; border-radius: 8px; color: white; margin-bottom: 8px; display: flex; justify-content: space-between;">
                    <b>{info['name']}</b> <span>{val:.1f}° / {info['limit']}° ({percent:.1f}%)</span></div>""", unsafe_allow_html=True)
        ...
        if low_parts:
            st.warning(f"⚠️ 달성도가 낮은 **{len(low_parts)}개 부위** 집중 개선 프로그램입니다.")
            display_parts = low_parts
        else:
            st.success("✨ 모든 관절 양호! 전신 관리 프로그램을 추천합니다.")
        ...
        for row in rows:
            ...
            for idx, part in enumerate(row):
                ...
                with cols[idx]:
                    if (val / info['limit']) < 0.7: st.error(f"**{part.upper()} 집중관리**")
                    else: st.info(f"**{part.upper()} 유지관리**")
